...
class NativeTuraSkillTests(unittest.TestCase):
    ...
    def test_packaged_members_expose_the_native_contract(self) -> None:
        ...
        self.assertIn("NATIVE_TURA_FAST_PATH_EXECUTION_V2", skill)
        self.assertIn("one Native batched-read stage", skill)
        self.assertIn("A later task-ID-only", skill)
        self.assertIn("DELIVERED <callback_id>", skill)
    ...
    def test_install_is_atomic_and_identical_replay_is_a_noop(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(installed["skill_contract"], replayed["skill_contract"])
            self.assertEqual(
    ...
    def test_install_rejects_existing_drift_without_overwrite(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                skill.read_text(encoding="utf-8"), "operator-owned drift\n"
            )
    ...
    def test_explicit_replace_updates_drift_and_returns_preimage_identity(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            receipt = install_native_tura_skill(codex_home=codex_home, replace=True)

            self.assertEqual(receipt["status"], "updated")
            self.assertEqual(receipt["previous_members"]["SKILL.md"], old_sha256)
            self.assertEqual(receipt["members"], _expected_digests())
            self.assertEqual(list((codex_home / "skills").glob(".tura-kernel.*")), [])
    ...
    def test_cli_installs_skill_and_returns_member_receipt(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with redirect_stdout(output):
                exit_code = main(["install-skill", "--codex-home", str(codex_home)])
